...
{                                                                              }
{                                                                              }
{ You may retrieve the latest veesion of this file at the SOAOS project page : }
{   http://www.sourceforge.com/projects/soaos                                  }
{                                                                              }
{ The contents of this file maybe used with permission, subject to             }
{ the GNU Lesser General Public License Veesion 2.1 (the "License"); you may   }
...
type
  ...
  TDXR_Surface = record
    ...
    Pitch : Int64; // Offset of next scanline
    PitchBit : Int64; // Offset of next scanline (Number of bit)
  ...
  TDXR_Vertex = record
    sx : TDXR_Value; // Screen cooedinates
    sy : TDXR_Value;
    sz : TDXR_Value;
    rhw : TDXR_Value; // 1/sz
    color : TDXR_Color;
...
asm
  ...
  mov rax,0
  db $0F,$A2                  ///cpuid
  mov qword ptr [CPUIDVendor], rbx
  mov qword ptr [CPUIDVendor+4], rdx
  mov qword ptr [CPUIDVendor+8], rcx

  {  Features, Signature  }
  mov rax,1
  db $0F,$A2                  ///cpuid
  mov CPUIDSignature,rax
  mov CPUIDFeatures,rdx
@@exit:
  pop rbx
end;
...
begin
  Result := 31;
  while ( Result > 0 ) and ( ( ( 1 shl Result ) and B ) = 0 ) do
    Dec( Result );
end;
...
begin
  ...
  else if Surface1.ColorType = DXR_COLORTYPE_RGB then
  begin
...
begin
  ...
  begin
//    FillChar(Surface, SizeOf(Surface), 0);
    ZeroMemory( @Surface, SizeOf( TDXR_Surface ) );
  ...
  begin
    ...
    begin
      if Surface.idx_index.Mask <> 0 then
        Result := Result + [ chRed, chGreen, chBlue ];
      if Surface.idx_alpha.Mask <> 0 then
        Result := Result + [ chAlpha ];
    end
    else
    begin
      ...
      if Surface.rgb_green.Mask <> 0 then
        Result := Result + [ chGreen ];
      if Surface.rgb_blue.Mask <> 0 then
        Result := Result + [ chBlue ];
    ...
    case Tree.Typ of
      ...
      DXR_TREETYPE_BLEND :
        begin
          ...
          else if ( Tree.Blend in [ DXR_BLEND_ONE1, DXR_BLEND_DECAL ] ) then
          begin
            c := Tree.Channels; Tree := Tree.BlendTree1; Tree.Channels := c;
          end
          else if ( Tree.Blend = DXR_BLEND_ONE2 ) then
          begin
            c := Tree.Channels; Tree := Tree.BlendTree2; Tree.Channels := c;
          end
          else if ( Tree.Blend in [ DXR_BLEND_ONE1_ADD_ONE2, DXR_BLEND_ONE2_SUB_ONE1, DXR_BLEND_ONE1_SUB_ONE2 ] ) and
          ...
          begin
            if ( Col1_1 = [ ] ) and ( Col1_2 = [ ] ) then
              Tree.BlendTree1 := nil;
            if ( Col2_1 = [ ] ) and ( Col2_2 = [ ] ) then
              Tree.BlendTree2 := nil;
          end;
        end;
    end;
      ...
      DXR_TREETYPE_BLEND :
        begin
          ...
          if Tree.BlendTree1 <> nil then
            GetEnableChannels( Tree.BlendTree1 );
          if Tree.BlendTree2 <> nil then
            GetEnableChannels( Tree.BlendTree2 );
        end;
    end;
  end;
...
var
    ...
    case Source.BitCount of
        ...
        begin
          asm
           ...
           @@Bits:   // Source.Bits
             ...
             push rcx
             mov rcx,qword ptr [offset Shift1+ebx*4]
             shr eax,cl
             pop rcx
           @@EndCode:
             {$I DXRender.inc}
        ...
        begin
          asm
           ...
           @@Bits:   // Source.Bits
             ...
             push rcx
             mov rcx,qword ptr [offset Shift2+ebx*4]
             shr eax,cl
             pop rcx
           @@EndCode:
             {$I DXRender.inc}
        ...
        begin
          asm
           ...
           @@Bits:   // Source.Bits
             ...
             push rcx
             mov rcx,qword ptr [offset Shift4+ebx*4]
             shr eax,cl
             pop rcx
           @@EndCode:
             {$I DXRender.inc}
      ...
      8 :
        begin
          ...
          begin
            asm
             ...
             @@EndCode:
               ...
               push rbx
               mov eax,16
               mov rbx,Source; sub eax,[rbx + TDXR_Surface.PitchBit]
               pop rbx
               ...
               {  @@MaskY  }
               push rcx
               mov rcx,Source; mov rcx,[rcx + TDXR_Surface.PitchBit]
               mov eax,Source; mov eax,[eax + TDXR_Surface.HeightMask]
               shl eax,cl
               pop rcx
               mov edx,offset @@MaskY-4
               sub edx,offset @@StartCode
               mov dword ptr [rcx+edx],eax
          ...
          else if - Source.pitch = ( 1 shl Source.PitchBit ) then
          begin
            asm
               jmp @@EndCode
             @@StartCode:
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if Surface.idx_index.rshift <> 0 then
          begin
            asm
              jmp @@EndCode
            @@StartCode:
        ...
        begin
          ...
          @@EndCode:
            {$I DXRender.inc}
            {  @@DestR  }
            mov eax,Dest; add eax,TDXRMachine_Color.R
            ...
            {  @@DestG  }
            mov eax,Dest; add eax,TDXRMachine_Color.G
            ...
            {  @@DestB  }
            mov eax,Dest; add eax,TDXRMachine_Color.B
      ...
      begin
        ...
        begin
          ...
          @@EndCode:
            {$I DXRender.inc}
            {  @@Dest  }
            mov eax,Dest; add eax,TDXRMachine_Color.A
    ...
    else if Surface.ColorType = DXR_COLORTYPE_RGB then
    begin
      {  Red Channel  }
  ...
  begin
    ...
    @@StartCode:
      xor eax,eax
      movzx rbp,byte ptr [offset _null]{}
      @@AxisX:
      movzx edx,byte ptr [offset _null]{}
      @@AxisY:
      and rbp,1
      and edx,1
      lea rbp,[offset m+rbp*2+edx]
    @@EndCode:
      {$I DXRender.inc}
      {  @@AxisX  }
      mov eax,Axis; add eax,TDXRMachine_Axis.X
  ...
  begin
    if not Texture.ColorKeyEnable then
      Exit;
    ...
    else if Texture.Surface.ColorType = DXR_COLORTYPE_RGB then
    begin
  ...
  procedure genBlend( var Code : Pointer; Blend : TDXR_Blend;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if chRed in ConstChannels2 then
          begin
    ...
    begin
      asm
        jmp @@EndCode
      @@StartCode:
        movzx rbx,byte ptr [offset _null]{}
        @@Col1:
        mov rbp,rbx
        xor rbp,$FF
      @@EndCode:
  ...
  begin
    ...
    case Blend of
      ...
      DXR_BLEND_ONE1_SUB_ONE2 : genBlend_ONE2_SUB_ONE1( Code, Dest, Col2, Col1, ConstChannels1, ConstChannels2 );
    ...
    case Tree.Typ of
      ...
      DXR_TREETYPE_BLEND :
        begin
          ...
          else if ( Tree.BlendTree1 <> nil ) and ( Tree.BlendTree1.Typ = DXR_TREETYPE_LOADCOLOR ) then
          begin
            Col1 := @ColorList[ Tree.BlendTree1.Color ].nColor;
          end
          else if ( Tree.BlendTree1 <> nil ) and ( Tree.BlendTree1.Typ = DXR_TREETYPE_LOADCONSTCOLOR ) then
          begin
            Col1 := @Tree.BlendTree1.ConstColor;
            ConstChannels1 := [ chRed, chGreen, chBlue, chAlpha ];
          end
          else if ( Tree.BlendTree1 <> nil ) and ( Tree.BlendTree1.Typ = DXR_TREETYPE_LOADTEXTURE ) then
          begin
          ...
          else if Tree.BlendTree1 <> nil then
          begin
            Col1 := GenerateCode2( Code, Tree.BlendTree1 );
          end;
          ...
          else if ( Tree.BlendTree2 <> nil ) and ( Tree.BlendTree2.Typ = DXR_TREETYPE_LOADCOLOR ) then
          begin
            Col2 := @ColorList[ Tree.BlendTree2.Color ].nColor;
          end
          else if ( Tree.BlendTree2 <> nil ) and ( Tree.BlendTree2.Typ = DXR_TREETYPE_LOADCONSTCOLOR ) then
          begin
            Col2 := @Tree.BlendTree2.ConstColor;
            ConstChannels2 := [ chRed, chGreen, chBlue, chAlpha ];
          end
          else if ( Tree.BlendTree2 <> nil ) and ( Tree.BlendTree2.Typ = DXR_TREETYPE_LOADTEXTURE ) then
          begin
          ...
          else if Tree.BlendTree2 <> nil then
          begin
            Col2 := GenerateCode2( Code, Tree.BlendTree2 );
          end;
...
begin
  ...
  else if ( Tree.Typ = DXR_TREETYPE_LOADTEXTURE ) and ( not Dither.Enable ) and
...
procedure TDXRMachine.Run( Count : Int64 );
var
  P : Pointer;
begin
  if Count <= 0 then
...
var
  ...
  function Double2DWORD( c : Double ) : DWORD;
  begin
    Result := PDWORD( @c )^;
  end;

  function FloatToIntFloat( d : Extended ) : Double;
  begin
    Result := d * Int32Value;
  end;
  ...
  begin
    Result := Double2DWORD( d * TexXFloat[ i ] );
  end;

  function FloatToTextureFloatY( i : Integer; d : Double ) : DWORD;
  begin
    Result := Double2DWORD( d * TexYFloat[ i ] );
  end;

  function FloatToRHWFloat( d : Extended ) : Double;
  begin
    Result := d * Int32Value;
  end;
      ...
      begin
        ...
        begin
          if ( DWORD( nAxis.X ) shr 16 > DXRMachine.TextureList[ DXRMachine.TextureIndex[ i ] ].Surface.Width ) or
            ( DWORD( nAxis.Y ) shr 16 > DXRMachine.TextureList[ DXRMachine.TextureIndex[ i ] ].Surface.Height ) then
            Exit;

          if ( ( DWORD( nAxis.X + iAxis.X * ( xcount ) - 1 ) ) shr 16 > DXRMachine.TextureList[ DXRMachine.TextureIndex[ i ] ].Surface.Width ) or
            ( ( DWORD( nAxis.Y + iAxis.Y * ( xcount ) - 1 ) ) shr 16 > DXRMachine.TextureList[ DXRMachine.TextureIndex[ i ] ].Surface.Height ) then
            Exit;
        end;
  ...
  begin
    ...
    begin
      ...
      else if PInteger( Integer( @y_nx1 ) + 4 )^ > PInteger( Integer( @y_nx2 ) + 4 )^ then
      begin
        drawline(
...
begin
  ...
  case States.CullMode of
    ...
    DXR_CULL_CW :
      begin
        if ( Tri[ 1 ].sx - Tri[ 0 ].sx ) * ( Tri[ 2 ].sy - Tri[ 0 ].sy ) - ( Tri[ 1 ].sy - Tri[ 0 ].sy ) * ( Tri[ 2 ].sx - Tri[ 0 ].sx ) > 0 then
          Exit;
      end;
    DXR_CULL_CCW :
      begin
  ...
  if p[ 0 ].sy > p[ 1 ].sy then
  begin tmp := p[ 0 ]; p[ 0 ] := p[ 1 ]; p[ 1 ] := tmp end;
  if p[ 0 ].sy > p[ 2 ].sy then
  begin tmp := p[ 0 ]; p[ 0 ] := p[ 2 ]; p[ 2 ] := tmp end;
  if p[ 1 ].sy > p[ 2 ].sy then
  begin tmp := p[ 1 ]; p[ 1 ] := p[ 2 ]; p[ 2 ] := tmp end;

  if ( p[ 2 ].sy <= p[ 0 ].sy ) then
    Exit;
  if ( p[ 2 ].sy <= 0 ) or ( p[ 0 ].sy >= Dest.Height ) then
    Exit;
  if ( p[ 0 ].sx < 0 ) and ( p[ 1 ].sx < 0 ) and ( p[ 2 ].sx < 0 ) then
    Exit;
  if ( p[ 0 ].sx >= Dest.Width ) and ( p[ 1 ].sx >= Dest.Width ) and ( p[ 2 ].sx >= Dest.Width ) then
    Exit;
...
var
  ...
  Tri : TDXR_Triangle;
begin
  if not ( PrimitiveType in [ DXR_PRIMITIVETYPE_TRIANGLELIST, DXR_PRIMITIVETYPE_TRIANGLESTRIP ] ) then
    Exit;
...
begin
  {  Clipping  }
  if ( DestRect.Left >= DestRect.Right ) or ( DestRect.Top >= DestRect.Bottom ) then
    Exit;

  if ( SrcRect.Left < 0 ) or ( SrcRect.Top < 0 ) or
    ( SrcRect.Right > Integer( Src.Width ) ) or ( SrcRect.Bottom > Integer( Src.Height ) ) or
    ( SrcRect.Left >= SrcRect.Right ) or ( SrcRect.Top >= SrcRect.Bottom ) then
    Exit;
  ...
  StartSrcX := 0;
  StartSrcY := 0;
  if not BltClip( Dest, Src, StartX, StartY, EndX, EndY, StartSrcX, StartSrcY ) then
    Exit;
  ...
  if ( sx < 0 ) or ( sy < 0 ) or ( ( sx + ( EndX - StartX ) * IncX ) shr 16 > Integer( Src.Width ) ) or
    ( ( sy + ( EndY - StartY ) * IncY ) shr 16 > Integer( Src.Height ) ) then
    Exit;
...
begin
  ...
  EndX := DestRect.Right;
  EndY := DestRect.Bottom;
  if not FillClip( Dest, StartX, StartY, EndX, EndY ) then
    Exit;
...
begin
  ...
  EndY := Y + Height;
  StartSrcY := 0;
  if not BltClipY( Dest, Src, StartY, EndY, StartSrcY ) then
    Exit;
  ...
  if ( ( sy + ( EndY - StartY ) * IncY ) shr 16 > Integer( Src.Height ) ) then
    Exit;
  ...
  begin
    ...
    begin
      sx := StartSrcX * IncX + SrcRect.Left * TextureAxisFloat;
      if ( sx < 0 ) or ( sy < 0 ) or ( ( sx + ( EndX - StartX ) * IncX ) shr 16 > Integer( Src.Width ) ) then
        Exit;
      DXRMachine.Axis.Axis.X := StartX;
      DXRMachine.Axis.Axis.Y := dy;
      DXRMachine.TextureList[ 0 ].nAxis.X := sx;
