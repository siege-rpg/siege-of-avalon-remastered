...
begin
  ...
  begin
    ...
    BltFx.dwFillColor := 0;
    WrapperBlt( lpDDSBack, rect( X, Y, X + Width, Y + Height ), nil, rect( 0, 0, 0, 0 ), DDBLT_COLORFILL + DDBLT_WAIT, BltFx );
    ...
    WrapperBlt( lpDDSBack, rect( X + 1, Y + 1, X + BarWidth, Y + Height - 1 ), nil, rect( 0, 0, 0, 0 ), DDBLT_COLORFILL + DDBLT_WAIT, BltFx );
  end;
end;
...
{$IFDEF DirectX}
      WrapperBltFast( lpDDSBack, X0, PosY - MsgHeight, MsgImage, Rect( 0, 0, MsgWidth, MsgHeight ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
{$ENDIF}
{$IFNDEF DirectX}
