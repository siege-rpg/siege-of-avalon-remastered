...
{$ENDIF}
  try
    ...
    WrapperBltFast( lpDDSBack, 103, 105, DXBack, Rect( 103, 105, 123, 321 ), DDBLTFAST_WAIT );
    WrapperBltFast( lpDDSBack, 188, 105, DXBack, Rect( 188, 105, 210, 315 ), DDBLTFAST_WAIT );
    WrapperBltFast( lpDDSBack, 581, 414, DXBack, Rect( 581, 414, 581 + 81, 414 + 57 ), DDBLTFAST_WAIT );
   //clear text
    WrapperBltFast( lpDDSBack, 10, 338, DXBack, Rect( 10, 338, 587, 470 ), DDBLTFAST_WAIT );
    i := 0;
    j := 0;
    while i < 16 do
    begin
      ...
      begin //if over an Arrow
        ArrowInfo( i );
        if i < 8 then
          WrapperBltFast( lpDDSBack, ArrowRect[ i ].rect.left, ArrowRect[ i ].rect.top, DXLeftArrow, Rect( 0, 0, 20, 15 ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT ) //plot the highlight
        else
          WrapperBltFast( lpDDSBack, ArrowRect[ i ].rect.left - 4, ArrowRect[ i ].rect.top, DXRightArrow, Rect( 0, 0, 20, 15 ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT ); //plot the highlight
    ...
    begin //we arent over anything else- check back button
      ...
      begin //over back button
        ...
        WrapperBltFast( lpDDSBack, 581, 414, DXBackToGame, Rect( 0, 0, 81, 57 ), DDBLTFAST_WAIT );
      end;
    end;
...
{$ENDIF}
  ...
  //clear the back down to the text - but dont clear the info block
    WrapperBltFast( lpDDSBack, 0, 0, DXBack, Rect( 0, 0, 677, 367 ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
