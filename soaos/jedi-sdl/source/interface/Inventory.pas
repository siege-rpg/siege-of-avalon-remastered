    ...
    begin //if no piece is being dragged pick one up
      ...
      begin //left arrow for ground
        ...
        begin //get the prev item on the ground and show it
          ...
          //WrapperBltFast( lpDDSBack, pTempItems(GroundOrderList.Items[j]).InvX, pTempItems(GroundOrderList.Items[j]).InvY, DXBack, Rect(pTempItems(GroundOrderList.Items[j]).InvX, pTempItems(GroundOrderList.Items[j]).InvY, pTempItems(GroundOrderList.Items[j]).InvX + pTempItems(GroundOrderList.Items[j]).W, pTempItems(GroundOrderList.Items[j]).InvY + pTempItems(GroundOrderList.Items[j]).H), DDBLTFAST_WAIT);
            WrapperBltFast( lpDDSBack, 275, 384, DXBack, Rect( 275, 384, 353, 416 ), DDBLTFAST_WAIT );
      ...
      begin //over the ground slot
        ...
        begin
          ...
          begin
            WrapperBltFast( lpDDSBack, 275, 384, DXBack, Rect( 275, 384, 353, 416 ), DDBLTFAST_WAIT ); //clean the box
            if GroundOrderList.Count > 1 then
            begin //get the next item on the ground and show it
...
//          else if (X > 250) and Locked then begin
    ...
    //make sure it isn't a quest item- cant drop those
      ...
      //DebugPrint(IntToStr(TopGroundIndex));
        ...
        WrapperBltFast( lpDDSBack, 276, 386, DXBack, Rect( 276, 386, 353, 416 ), DDBLTFAST_WAIT ); //clean out the Ground box
      ...
      begin //is the mouse over on the right (body side) of the screen
        ...
        //plot dropped item in slot
          ...
          //assign Slot to dropped item and CurrentSelectedItem to the item we got from the slot; swap them
            ...
            // pTempItems(ItemList.Items[CurrentSelectedItem]).BodySlot:=-1;
              //Plot relevant text
              WrapperBltFast( lpDDSBack, ClearLeft, ClearTop, DXBack, Rect( 20, ClearTop, ClearRight, ClearBottom ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT ); //clean up before we plot test
              if UseSmallFont then
                pText.PlotTinyTextBlock( GetSlotText, ClearLeft, ClearRight, SmlMsg, Alpha )
              else
                pText.PlotText( GetSlotText, ClearLeft, LrgMsg, Alpha );
          ...
          begin //The slot was empty, so - clear the CurrentSelectedItem item and clear the text
            WrapperBltFast( lpDDSBack, ClearLeft, ClearTop, DXBack, Rect( ClearLeft, ClearTop, ClearRight, ClearBottom ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
            SlotCoord[ i ].UsedBy := CurrentSelectedItem;
            CleanUpLegalSlots;
            CurrentSelectedItem := -1;
            ContainCursor( 0 );
      ...
      begin //check inventory side
        ...
        //Ty := Integer((Y - 47 - (pTempItems(ItemList.Items[CurrentSelectedItem]).H div 2)) div 26) * 26 + 57;
          ...
          CurrentSelectedItem := -1;
          ContainCursor( 0 );
          WrapperBltFast( lpDDSBack, ClearLeft, ClearTop, DXBack, Rect( ClearLeft, ClearTop, ClearRight, ClearBottom ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT ); //erase any message on the screen
        end
        else
...
//            pText.PlotText((GetSlotText + txtMessage[7]), ClearLeft, LrgMsg,Alpha);
    ...
    //Clean up arrows and back to game
      WrapperBltFast( lpDDSBack, 261, 394, DXBack, Rect( 261, 394, 261 + 14, 394 + 15 ), DDBLTFAST_WAIT );
      WrapperBltFast( lpDDSBack, 354, 396, DXBack, Rect( 354, 396, 354 + 11, 396 + 11 ), DDBLTFAST_WAIT );
      WrapperBltFast( lpDDSBack, 595, 416, DXBack, Rect( 595, 416, 595 + 74, 416 + 46 ), DDBLTFAST_WAIT );
      if CurrentSelectedItem = -1 then
      begin //If we arent over an item then check arrows and back button
        ...
        begin //over left arrow
          ...
          WrapperBltFast( lpDDSBack, 261, 394, DXLeftArrow, Rect( 0, 0, 14, 15 ), DDBLTFAST_WAIT );
        ...
        begin //over right arrow
          //plot highlighted arrow
          WrapperBltFast( lpDDSBack, 354, 396, DXRightArrow, Rect( 0, 0, 11, 11 ), DDBLTFAST_WAIT );
          //plot a bit of informative text
          WrapperBltFast( lpDDSBack, ClearLeft, ClearTop, DXBack, Rect( ClearLeft, ClearTop, ClearRight, ClearBottom ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT ); //clean up before we plot text
          if UseSmallFont then
        ...
        begin //over back button
          ...
          WrapperBltFast( lpDDSBack, 595, 416, DXBackToGame, Rect( 0, 0, 74, 46 ), DDBLTFAST_WAIT );
...
{$ENDIF}
  try
   ...
   //WrapperBltFast( lpDDSBack, 20,40,DXBack,rect(20,40,238,423), DDBLTFAST_WAIT);
    for i := 0 to 11 do
    begin
      for j := 0 to 13 do
      begin
