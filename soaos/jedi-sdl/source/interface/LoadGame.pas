...
//  frmMain.OnMouseDown := FormMouseDown;
  ...
  //transparent color
    ...
    begin
      ...
      WrapperBltFast( DXBack, ldLoadDarkRect.Left, ldLoadDarkRect.Top, DXTemp, rect( 0, 0, BM.width, BM.height ), DDBLTFAST_WAIT );
    end
    else
    begin
      BM.LoadFromFile( InterfacePath + 'ldSaveDark.bmp' );
      DXTemp := DDGetImage( lpDD, BM, InvisColor, False );
      WrapperBltFast( DXBack, ldSaveDarkRect.Left, ldSaveDarkRect.Top, DXTemp, rect( 0, 0, BM.width, BM.height ), DDBLTFAST_WAIT );
    end;
    ...
    begin
      ...
      DXTemp := DDGetImage( lpDD, BM, InvisColor, False );
      WrapperBltFast( DXBack, ldLoadUpperRect.Left, ldLoadUpperRect.Top, DXTemp, rect( 0, 0, BM.width, BM.height ), DDBLTFAST_WAIT );
    end
    else
    begin
      BM.LoadFromFile( InterfacePath + 'ldSaveUpper.bmp' );
      DXTemp := DDGetImage( lpDD, BM, InvisColor, False );
      WrapperBltFast( DXBack, ldSaveUpperRect.Left, ldSaveUpperRect.Top, DXTemp, rect( 0, 0, BM.width, BM.height ), DDBLTFAST_WAIT );
    end;
    ...
    begin
      ...
      begin //not on screen, set coll rect offscreen
        ...
        begin //if current highlight scrolled offscreen, drop highlight
          CurrentSelectedListItem := -1;
//           WrapperBltFast( lpDDSBack, 114,257,DXBack,rect(114,257,344,421),DDBLTFAST_WAIT);
//           WrapperBltFast( lpDDSBack, 111,65,DXBack,rect(111,65,344,231),DDBLTFAST_WAIT);
        end;
      end;
    end; //end for
  except
  ...
  try
    ...
    begin
      ...
      nRect := pItem( SelectRect.Items[ CurrentSelectedListItem ] ).Rect;
         //WrapperBltFast( lpDDSBack, X1,Y1,DXBack,rect(X1,Y1,586,Y1+24),DDBLTFAST_WAIT);
      WrapperBltFast( lpDDSBack, nRect.left - 10, nRect.top - 5, DXBack, rect( nRect.left - 10, nRect.top - 5, nRect.right, nRect.bottom - 5 ), DDBLTFAST_WAIT );
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                     //WrapperBltFast( lpDDSBack, 114,257,DXBack,rect(114,257,341,421),DDBLTFAST_WAIT);
                WrapperBltFast( lpDDSBack, 111, 65, DXBack, rect( 111, 65, 341, 231 ), DDBLTFAST_WAIT );
              end;
...
{$ENDIF}
  try
    ...
    begin
      //clear menu
  //    WrapperBltFast( lpDDSBack, 368,52,DXBack,rect(368,52,671,386),DDBLTFAST_WAIT);
      //clear Delete
      //WrapperBltFast( lpDDSBack, 8,417,DXBack,rect(8,417,64,436),DDBLTFAST_WAIT);
      //clear Cancel
      WrapperBltFast( lpDDSBack, ldCancelRect.Left, ldCancelRect.Top, DXBack, rect( ldCancelRect.Left, ldCancelRect.Top, ldCancelRect.Left + ldCancelRect.Right, ldCancelRect.Top + ldCancelRect.Bottom ), DDBLTFAST_WAIT );
      //Clear Load
      WrapperBltFast( lpDDSBack, DXLoadRect.Left, DXLoadRect.Top, DXBack, rect( DXLoadRect.Left, DXLoadRect.Top, DXLoadRect.Left + DXLoadRect.Right, DXLoadRect.Top + DXLoadRect.Bottom ), DDBLTFAST_WAIT );
  ...
  {    for i:=0 to SelectRect.count -1 do begin
      ...
      begin //load game
        if CurrentSelectedListItem >= 0 then
          WrapperBltFast( lpDDSBack, DXLoadRect.Left, DXLoadRect.Top, DXLoad, rect( 0, 0, DXLoadRect.Right, DXLoadRect.Bottom ), DDBLTFAST_WAIT );
      end
      ...
      begin //cancel
        WrapperBltFast( lpDDSBack, ldCancelRect.Left, ldCancelRect.Top, DXCancel, rect( 0, 0, ldCancelRect.Right, ldCancelRect.Bottom ), DDBLTFAST_WAIT );
      end;
...
{$ENDIF}
  try
    //clear Cancel
    WrapperBltFast( lpDDSBack, 95, 443, DXBack, rect( 95, 443, 95 + 165, 443 + 58 ), DDBLTFAST_WAIT );
    //Clear Load
    WrapperBltFast( lpDDSBack, 581, 445, DXBack, rect( 581, 445, 581 + 121, 445 + 54 ), DDBLTFAST_WAIT );
  except
    on E : Exception do
      Log.log( FailName, E.Message, [ ] );
  end;
...
{$ENDIF}
  ...
  //WrapperBltFast( lpDDSBack, 369, 431, DXBorders, Rect(0, 0, BM.width, BM.Height), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT);
    nRect := pItem( SelectRect.Items[ CurrentSelectedListItem ] ).Rect;
    WrapperBltFast( lpDDSBack, nRect.left - 10, nRect.top + 32, DXBorders, Rect( 0, 0, BM.width, BM.Height ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
...
begin
  ...
  nRect := pItem( SelectRect.Items[ CurrentSelectedListItem ] ).Rect;
  WrapperBltFast( lpDDSBack, nRect.left - 10, nRect.top + 32, DXBorders, Rect( 0, 0, BM.width, BM.Height ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
  WrapperBltFast( lpDDSBack, nRect.left - 10, nRect.top + 32 + 75, DXok, Rect( 0, 0, 300, 42 ), DDBLTFAST_WAIT );
  DXBorders := nil;
...
{$ENDIF}
  try
    ...
    begin
      ...
      WrapperBltFast( lpDDSBack, 114, 257, DXTemp, rect( 0, 0, 225, 162 ), DDBLTFAST_WAIT );
      BM.free;
      DXTemp := nil;
    end
    else
    begin
      WrapperBltFast( lpDDSBack, 114, 257, DXBack, rect( 114, 257, 340, 420 ), DDBLTFAST_WAIT );
    end; //endif
