...
{$ENDIF}
  ...
  //Now for the Alpha'ed edges
    ...
    if IniFileFound and ( MaxPages > 0 ) then
    begin //(LogInfo.count > 15) then begin
      WrapperBltFast( lpDDSBack, 400, 424, DXPrev, Rect( 0, 0, 86, 29 ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
      WrapperBltFast( lpDDSBack, 500, 424, DXNext, Rect( 0, 0, 62, 27 ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
    end;
    ...
    begin //(LogInfo.count > 15) then begin
      ...
      begin //over prev
        ...
        begin
          ...
          WrapperBltFast( lpDDSBack, 0, 40, DXBack, Rect( 0, 40, 650, 415 ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
          WrapperBltFast( lpDDSBack, 20, 424, DXBack, Rect( 20, 424, 350, 450 ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
          pText.plotText( txtMessage[ 0 ] + inttostr( PageNumber + 1 ) + txtMessage[ 1 ] + inttostr( MaxPages + 1 ), 20, 424, 240 );
          ShowText( PageNumber );
        end;
      end;
...
{$ENDIF}
  try
    ...
    begin //(LogInfo.count > 15) then begin
      if PtinRect( rect( 400, 424, 400 + 86, 424 + 29 ), point( X, Y ) ) then
      begin //over prev
        WrapperBltFast( lpDDSBack, 400, 424, DXPrev2, Rect( 0, 0, 86, 29 ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
      end;
      if PtinRect( rect( 500, 424, 500 + 86, 424 + 29 ), point( X, Y ) ) then
      begin //over next
        WrapperBltFast( lpDDSBack, 500, 424, DXNext2, Rect( 0, 0, 62, 27 ), DDBLTFAST_SRCCOLORKEY or DDBLTFAST_WAIT );
      end;
    end;
    if PtinRect( rect( 588, 407, 588 + 77, 412 + 54 ), point( X, Y ) ) then
    begin //over back button
